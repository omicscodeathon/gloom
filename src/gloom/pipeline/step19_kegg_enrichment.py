...
def _run_enrichr(gene_list: list, label: str) -> tuple[pd.DataFrame, str]:
    try:
        import gseapy as gp
    except ImportError:
    ...
    if len(gene_list) < MIN_GENE_OVERLAP:
        reason = (
            f"Only {len(gene_list)} genes were available in the {label.lower()} set; "
            f"this is below the minimum overlap threshold of {MIN_GENE_OVERLAP} genes."
        )
    ...
    try:
        ...
        return pd.DataFrame(), f"Enrichr query failed for the {label.lower()} set: {exc}"
    
    df = enr.results.copy()
    if df.empty:
        reason = f"Enrichr returned no KEGG pathways for the {label.lower()} set."
    ...
    df = df[df.get("overlap_genes", pd.Series(0, index=df.index)) >= MIN_GENE_OVERLAP].copy()
    if df.empty:
        reason = (
            f"No KEGG pathways from the {label.lower()} set met the minimum overlap threshold "
            f"of {MIN_GENE_OVERLAP} genes; all detected overlaps were below {MIN_GENE_OVERLAP}."
        )
        log.warning(f"  [{label}] No pathways with overlap >= {MIN_GENE_OVERLAP} — returning empty result.")
        return pd.DataFrame(), reason
    ...
    if df.empty:
        reason = (
            f"KEGG returned pathways for the {label.lower()} set, but none matched the "
            "lung/cancer pathway filter."
        )
        log.warning(f"  [{label}] No lung/cancer pathways remained after filtering — returning empty result.")
        return pd.DataFrame(), reason
...
def run_kegg_enrichment() -> dict:
    ...
    for gene_list, label, out_csv in analyses:
        ...
        df, reason = _run_enrichr(gene_list, label)
        if not df.empty:
    ...
    for label, df in results.items():
        ...
        rows.append({
            ...
            "note": "" if len(sig) > 0 else (
                f"No {label.lower()} pathways passed the adjusted p-value cutoff of {PADJ_CUTOFF}."
            ),
        })
        continue
        rows.append({
