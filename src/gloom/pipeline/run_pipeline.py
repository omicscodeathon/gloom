...
Step numbering:
  ...
  2   Preprocessing / QC
  3   Harmonization
  4   Differential expression        ← P2.2: stricter DE thresholds applied
  5   Expression features
  6   Tumor co-expression network    ← P2.3: lower correlation cutoff applied
  6b  Normal co-expression network   ← P3.2: needed for differential features
  7   Tumor network features
  7b  Differential network features  ← P3.2: tumor − normal rewiring
  8   Feature integration            ← auto-includes 7b output if available
  9   Label construction
  10  Train / validation split
  11  Model training                 ← P2.1: Platt calibration applied
  11b PU Bagging (Mordelet-Vert)     ← P3.1: true PU learning
  12  Model evaluation               ← P2.4: multi-threshold confusion matrices
  13  Feature importance
  14  Gene ranking                   ← P1.1/P1.4: scaler fix + dual thresholds
  15  Network annotation
  16  Network export
  17  Interactive visualization
  18  Final report
  19  KEGG enrichment
  20  Independent cohort validation  ← P3.3: requires EXTERNAL_EXPR_FILE

NOTE — Batch correction redirect:
  If USE_BATCH_CORRECTION=True, after step 1b completes you must update
  config.py to point TUMOR_EXPR_HARMONIZED and NORMAL_EXPR_HARMONIZED
  to the batch-corrected files before step 4 runs.
  The pipeline will PAUSE and prompt you to do this if needed.
"""
...
#   step_key : sortable string used for --from / --only / --to matching
#   optional  : True  → warn on failure but continue
#               False → halt pipeline on failure
STEPS = [
    # ── Data ingestion ─────────────────────────────────────────────────────────
    ...
    # ── Independent validation (requires EXTERNAL_EXPR_FILE) ──────────────────
    ("20",  "Step 20  — Indep. validation",     "step15_independent_validation",
...
def _batch_correction_redirect_check():
    """
    After step1b, warn if config still points to un-corrected harmonized files
    when USE_BATCH_CORRECTION=True, so step4 uses the right inputs.
    """
    ...
    current_normal   = config.NORMAL_EXPR_HARMONIZED
...
def _key_sort(key):
    """Sort step keys so '06b' comes after '06' and before '07'."""
    base = key.rstrip("b")
    suffix = 1 if key.endswith("b") else 0
    return (int(base), suffix)


def _filter_steps(steps, from_key=None, to_key=None, only_key=None):
    if only_key:
        return [s for s in steps if s[0] == only_key]
...
def run_pipeline(from_key=None, to_key=None, only_key=None):
    steps = _filter_steps(STEPS, from_key, to_key, only_key)

    log.info("=" * 70)
    log.info("LUAD ML PIPELINE — FULL RUN  (phases 1–3 fixes applied)")
    ...
    for step_key, step_label, module_name, func_name, optional in steps:
        ...
        try:
            ...
            # Reload to pick up any config changes made mid-run
        ...
        # After step1b: warn if batch-correction redirect is needed
        if step_key == "01b":
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "            11, 11b, 12, 13, 14, 15, 16, 17, 18, 19, 20\n\n"
            "Examples:\n"
            "  python run_pipeline.py                  # full run\n"
            "  python run_pipeline.py --from 4         # resume from DE step\n"
            "  python run_pipeline.py --from 6 --to 8 # network → integration\n"
