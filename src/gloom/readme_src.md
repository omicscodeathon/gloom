...
- `pipeline/config.py` - central paths, thresholds, flags, and output locations
- `pipeline/run_pipeline.py` - master runner for full runs or selected step ranges
- `pipeline/requirements.txt` - direct script dependencies for the pipeline scripts
- `cli.py` - package CLI entrypoints such as `gloom prioritize` and `gloom run`
- `output.py` - reorganizes raw pipeline artifacts into the public package output layout
...
| `18` | `step18_final_report.py` | Write summary tables and summary figures, and attempt the text report | No |
...
- `results/reports/pipeline_report.txt` when Step `18` text-report generation succeeds
