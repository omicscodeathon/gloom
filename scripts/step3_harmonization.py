...
Steps: normalize symbols, find intersection, check LCGene retention,
subset both matrices, validate alignment.

FIX: normalize_gene_symbols now also handles numeric-string gene IDs
     that slipped through step1 (e.g. pure Entrez IDs stored as strings),
     and logs clearly when duplicates are collapsed after upper-casing.
...
def normalize_gene_symbols(df, label):
    """
    Upper-case and strip all gene index entries.
    Drops: empty strings, 'NAN'/'NA'/'NONE', pure-numeric strings.
    FIX: after upper-casing, collapses any newly created duplicates by
         keeping the row with the highest mean expression, and logs the count.
    """
    n_before = len(df)
    ...
    invalid_mask = (
        df.index.isin(["NAN", "", "NA", "NONE"])
        | df.index.str.fullmatch(r"\d+")
    )
    if invalid_mask.sum() > 0:
        log.warning(f"  [{label}] Dropping {invalid_mask.sum()} invalid symbols.")
        df = df.loc[~invalid_mask]

    # FIX: collapse duplicates that may arise after upper-casing
...
def check_cancer_gene_retention(common_genes, cancer_genes, label="harmonized"):
    ...
    log.info(f"  LCGene retained: {len(retained)} / {len(cancer_set)} ({pct:.1f}%)")
    if lost:
        log.warning(f"  LCGene genes lost: {', '.join(sorted(lost))}")
    return pd.DataFrame([{
        "stage":              label,
        "total_cancer_genes": len(cancer_set),
...
def plot_venn(tumor_genes, normal_genes, common_genes, out_path):
    n_t = len(tumor_genes)
    n_n = len(normal_genes)
    n_c = len(common_genes)
    ...
    ax.add_patch(ct)
    ax.add_patch(cn)
    ax.text(2.2, 3.0, f"{n_t - n_c:,}", ha="center", va="center",
            fontsize=14, fontweight="bold", color="steelblue")
    ax.text(7.8, 3.0, f"{n_n - n_c:,}", ha="center", va="center",
...
def run_harmonization():
    ...
    lcgene_report = check_cancer_gene_retention(common_genes, cancer_genes)
    ...
    plot_venn(
        tumor_expr.index, normal_expr.index, common_genes,
    ...
    lcgene_report.to_csv(config.PROCESSED_DIR / "lcgene_retention_report.csv", index=False)
