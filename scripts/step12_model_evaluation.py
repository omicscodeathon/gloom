...
def evaluate_single_model(model_name, model, use_scaled, X_val, X_val_scaled, y_val):
    ...
    # P1.2 FIX: Decouple threshold selection from threshold reporting.
    # Oracle threshold (selected to maximise F1 on this same validation set) is
    # an optimistic upper bound — reported as reference only, NOT as primary metric.
    # Fixed threshold = 0.15: sits between most models' oracle thresholds (0.10–0.28),
    # giving a better precision-recall balance than 0.10 for most models.
    ...
    # Fixed-threshold metrics (primary / honest)
    ...
    # Oracle-threshold metrics (reference only — selected on same val set)
    ...
    # Classification report (text) at fixed threshold
    ...
    log.info(f"  [{model_name:<22}] AUROC={auroc:.4f} AUPRC={auprc:.4f} "
             ...
             f"[oracle F1@{opt_thresh:.2f}={opt_f1:.4f}] "
    ...
    return {"model_name":model_name,"auroc":round(auroc,4),"auprc":round(auprc,4),
            ...
            "mcc_fixed":round(mcc_fixed,4),"precision_fixed":round(prec_fixed,4),
            "recall_fixed":round(rec_fixed,4),"f1_fixed":round(f1_fixed,4),
            "threshold_fixed":FIXED_THRESHOLD,
            # Oracle-threshold (reference only)
            "mcc_oracle":round(mcc_oracle,4),"precision_oracle":round(prec_oracle,4),
            "recall_oracle":round(rec_oracle,4),"f1_oracle":round(opt_f1,4),
...
def run_model_evaluation():
    ...
    # Weights come from CV AUPRC (models/cv_results.csv); falls back to equal weights.
    if len(eval_results) >= 2:
        ...
        if cv_path.exists():
            ...
            log.info("  cv_results.csv not found — using equal ensemble weights.")
        ...
        ens_mcc_oracle  = matthews_corrcoef(y_val, ens_y_pred_opt)
        ens_prec_oracle = precision_score(y_val, ens_y_pred_opt, zero_division=0)
        ens_rec_oracle  = recall_score(y_val, ens_y_pred_opt, zero_division=0)
        ...
        ens_result = {
            ...
            "mcc_fixed":round(ens_mcc_fixed,4),"precision_fixed":round(ens_prec_fixed,4),
            "recall_fixed":round(ens_rec_fixed,4),"f1_fixed":round(ens_f1_fixed,4),
            "threshold_fixed":FIXED_THRESHOLD,
            "mcc_oracle":round(ens_mcc_oracle,4),"precision_oracle":round(ens_prec_oracle,4),
            "recall_oracle":round(ens_rec_oracle,4),"f1_oracle":round(ens_opt_f1,4),
    ...
    scalar_keys = ["model_name","auroc","auprc","brier_score",
                   # Fixed-threshold (primary / honest)
                   "accuracy_fixed","mcc_fixed","precision_fixed","recall_fixed","f1_fixed","threshold_fixed",
                   # Oracle-threshold (reference only)
                   "mcc_oracle","precision_oracle","recall_oracle","f1_oracle","optimal_threshold",
    ...
    for result in eval_results:
        ...
        header = (f"Model: {result['model_name']}\n"
                  f"Fixed threshold: {result['threshold_fixed']}  "
                  f"Oracle threshold (ref): {result['optimal_threshold']}\n"
                  f"Accuracy@fixed: {result['accuracy_fixed']:.4f}\n"
                  f"{'-'*60}\n")
    ...
    for ax_i, result in enumerate(eval_results):
        ...
        axes_cm[ax_i].set_title(f"{result['model_name']}{star}\nF1@0.50={result['f1_fixed']:.4f}  MCC@0.50={result['mcc_fixed']:.4f}", fontsize=10)
