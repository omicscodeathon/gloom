...
def log2_transform(df, label):
    ...
    log.info(
        f"  [{label}] log2(x+1) applied. "
        f"Range: [{transformed.min().min():.3f}, {transformed.max().max():.3f}]"
    )
    return transformed
...
def validate_sample_overlap(expr_df, meta_df, label):
    """
    Align expression columns to metadata index, keeping only common samples.
    For tumor: tries TCGA 12-character ID truncation as a fallback.
    For normal: GTEx SAMPID format matches directly between expression and metadata.
    """
...
def plot_sample_distributions(df_before, df_after, label, out_path, n_samples=50):
    """
    Plot violin + box of per-sample median expression before and after QC.
    ...
    def _sample_medians(df):
        ...
        medians = df[cols].median(axis=0)
        # FIX: replace NaN medians (all-zero columns before log transform)
        #      so violinplot does not receive an all-NaN array
    ...
    for ax, medians, title in zip(
        ...
        ax.boxplot(
            medians, positions=[1], patch_artist=True,
...
def plot_gene_filter_summary(stats, out_path):
    stages = [
        "Raw loaded",
        "After low-expr filter",
        "After low-var filter",
...
def run_preprocessing() -> dict:
    ...
    gene_stats = {
        "tumor":  {"Raw loaded": tumor_expr_raw.shape[0]},
    ...
    # FIX: validate_sample_overlap is now called for normal as well,
    #      so normal_meta is properly trimmed to expression-matched samples
    #      and gene_stats["normal"]["After sample alignment"] reflects the
    ...
    # FIX: align normal expression columns to RNA-seq-only metadata rows
    normal_expr, normal_meta = validate_sample_overlap(normal_expr, normal_meta, "normal")
    ...
    plot_sample_distributions(
        tumor_expr_raw, tumor_expr, "tumor",
        config.FIGURES_DIR / "qc_tumor_sample_distributions.png",
    )
    plot_sample_distributions(
        normal_expr_raw, normal_expr, "normal",
    ...
    # ------------------------------------------------------------------
    # Save outputs  (retry on Windows file-lock / PermissionError)
    # ------------------------------------------------------------------
    def _save(df, path, **kwargs):
        path = Path(path)
    ...
    _save(tumor_expr,  config.TUMOR_EXPR_PROCESSED)
    _save(normal_expr, config.NORMAL_EXPR_PROCESSED)
    _save(tumor_meta,  config.PROCESSED_DIR / "tumor_metadata_processed.csv")
    _save(normal_meta, config.PROCESSED_DIR / "normal_metadata_processed.csv")
    _save(qc_summary,  config.PROCESSED_DIR / "qc_summary.csv", index=False)
